...
import { Camera, Shield, UserCheck, Loader2 } from "lucide-react";
import { useUploadTeacherAvatar } from "@/hooks/profile.hooks";
...
const TeacherProfileBanner: React.FC<TeacherProfileBannerProps> = ({
  firstName,
  lastName,
  username,
  role,
  ...
  const { mutate: uploadAvatar, isPending } = useUploadTeacherAvatar();
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-8 flex flex-col md:flex-row items-center md:items-start gap-8 shadow-sm relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-6 relative z-10 w-full text-center md:text-left">
        <div>
          <h2 className="text-2xl font-black text-[#000000] leading-tight uppercase">
            {firstName} {lastName}
          </h2>
          <p className="text-gray-500 font-medium capitalize">{role}</p>
        </div>
        <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-[#0e2e1d]/5 text-[#0e2e1d] rounded-lg border border-[#0e2e1d]/10">
            <UserCheck size={14} className="text-[#0e2e1d]" />
            <span className="text-xs font-bold tracking-tight">{currentClass || 'Senior Staff'}</span>
          </div>
        </div>
      </div>
    </div>
